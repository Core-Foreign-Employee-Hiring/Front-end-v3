...
export async function PATCH(request: Request, context: { params: Promise<{ resumeId: string }> }) {
  try {
    // Next.js 15에서는 params를 반드시 await 해야 합니다.
    const { resumeId } = await context.params
    const requestData: ResumeSelectionType = await request.json()
    ...
    const endpoint = `/api/v1/resumes/${resumeId}/selections`
