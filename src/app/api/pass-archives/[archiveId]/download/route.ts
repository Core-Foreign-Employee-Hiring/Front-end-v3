...
import { PassArchiveDownLoadFileType } from '@/types/pass-archive'
...
export async function GET(request: Request, context: { params: Promise<{ archiveId: string }> }) {
  try {
    ...
    const endpoint = `/api/v1/pass-archives/${archiveId}/download`
    console.log('최종 호출 API 경로:', endpoint)

    const result = await apiCallServer<PassArchiveDownLoadFileType[]>(endpoint, {
      method: 'GET',
    })
