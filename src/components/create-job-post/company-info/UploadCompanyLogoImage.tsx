...
export default function UploadCompanyLogoImage() {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      updateCreateJobPost('companyImageUrl', file)
    }
  }
  ...
  const handleRemoveFile = () => {
    updateCreateJobPost('companyImageUrl', null)
  ...
  return (
    <div className="flex flex-col gap-y-2">
      <Label label={'회사 로고 업로드'} isOption={true} />
      <Button
      ...
      <UploadItem file={createJobPost.companyImageUrl} onRemove={handleRemoveFile} />
